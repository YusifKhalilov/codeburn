...
final class AppStore: ObservableObject {
    ...
    @Published var selectedProvider: ProviderFilter = .all
    @Published var selectedPeriod: Period = .today
    @Published var selectedInsight: InsightMode = .trend
    @Published var accentPreset: AccentPreset = ThemeState.shared.preset {
        didSet { ThemeState.shared.preset = accentPreset }
    }
    @Published var showingAccentPicker: Bool = false
    @Published var currency: String = "USD"
    @Published var lastError: String?
    @Published var subscription: SubscriptionUsage?
    @Published var subscriptionError: String?
    @Published var subscriptionLoadState: SubscriptionLoadState = .idle
    @Published var capacityEstimates: [String: CapacityEstimate] = [:]
...
enum ProviderFilter: String, CaseIterable, Identifiable {
    ...
    var providerKeys: [String] {
        ...
        case .rooCode: return ["roo-code", "roo code"]
        case .kiloCode: return ["kilo-code", "kilocode"]
...
@MainActor extension Double {
    nonisolated func asCurrency() -> String {
    ...
    nonisolated func asCompactCurrency() -> String {
        let state = CurrencyState.shared
        return String(format: "\(state.symbol)%.2f", self * state.rate)
    }

    nonisolated func asCompactCurrencyWhole() -> String {
        let state = CurrencyState.shared
