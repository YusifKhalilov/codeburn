...
enum SubscriptionError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .noCredentials: return "No Claude OAuth credentials found"
        case .credentialsInvalid: return "Claude OAuth credentials malformed"
        case let .refreshFailed(code, body): return "Token refresh failed (\(code))\(body.map { ": \($0)" } ?? "")"
        case let .usageFetchFailed(code, body): return "Usage fetch failed (\(code))\(body.map { ": \($0)" } ?? "")"
        case let .decodeFailed(err): return "Decode failed: \(err.localizedDescription)"
        }
    }
}
